...
class CarParkDisplay:
    ...
    def __init__(self,root):
        ...
        #updater = threading.Thread(target=self.check_updates) #deleted
       # updater.daemon = True
        #updater.start()
        self.window.show()
        self._provider=None
    ...
    def data_provider(self,provider):
        if isinstance(provider,CarparkDataProvider):
            ...
            provider._display_callback = self.update_display  #added
        self.update_display()
...
class CarDetectorWindow:
    ...
    # def timestamp(self):                         #added
    #     return self.time.localtime()
...
#        print("Car goes out")
        for listener in self.listeners:
            listener.outgoing_car(self.current_license) #added
...
if __name__ == '__main__':
    ...
    manager = car_manager1.CarparkManager(display.update_display)
   ...
   # display=CarParkDisplay(root)
    ...
    root.mainloop()
    manager._log_manager.export_as_dataframe()
